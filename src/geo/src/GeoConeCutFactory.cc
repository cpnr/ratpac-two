...
G4VPhysicalVolume *GeoConeCutFactory::Construct(DBLinkPtr table) {
  ...
  G4double side_L = table->GetD("side_L");

  numZPlanes = G4int(z.size());

  if ((z.size() != r_max.size()) || (z.size() != r_min.size()) || (r_max.size() != r_min.size())) {
    Log::Die(
        "GeoConeCutFactory::ConstructSolid: Tables z, r_max and r_min "
  ...
  G4Box * GBox = new G4Box(volume_name+"_cutBox", 0.5*side_L*CLHEP::mm ,0.5*side_L*CLHEP::mm, (z[1]-z[0])*CLHEP::mm);

  G4IntersectionSolid * newVolume =  new G4IntersectionSolid(volume_name+"_ConeCut",  BaseSolid, GBox);
  
  return GeoSolidArrayFactoryBase::Construct(newVolume,table);
}
